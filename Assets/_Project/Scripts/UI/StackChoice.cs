...
public class StackChoice : MonoBehaviour
{
    ...
    private void ShowLevelUpChoices()
    {
        ...
        {
            Debug.LogWarning("[StackChoice] Stack_Choice_UI reference is missing.");
            return;
        }
    ...
    {
        Debug.Log($"[StackChoice] SelectChoice called. index={index}, currentChoices={currentChoices.Count}, panelActive={(stackChoicePanel != null && stackChoicePanel.activeInHierarchy)}");

        if (selectionInProgress)
        {
            Debug.Log("[StackChoice] SelectChoice ignored: selectionInProgress=true");
            return;
        }

        if (index < 0 || index >= currentChoices.Count)
        {
            Debug.LogWarning($"[StackChoice] SelectChoice ignored: invalid index {index}");
            return;
        }
    ...
    private void ApplyStat(StackStatType statType)
    {
        ...
        if (playerStats == null)
        {
            Debug.LogWarning("[StackChoice] PlayerStats instance not found.");
            return;
        }

        CharacterStats runtimeStats = playerStats.RuntimeStats;
        string source = $"StackChoice_{statType}_{selectionSerial++}";

        switch (statType)
        {
            case StackStatType.AttackPower:
                runtimeStats.AddModifier(CharacterStatType.AttackPower, attackPowerFlatIncrease, CharacterStatModifierMode.Flat, source);
                break;
            case StackStatType.AttackRange:
                runtimeStats.AddModifier(CharacterStatType.Range, attackRangePercentIncrease / 100f, CharacterStatModifierMode.PercentAdd, source);
                break;
            case StackStatType.AttackSpeed:
                runtimeStats.AddModifier(CharacterStatType.AttackSpeed, attackSpeedPercentIncrease / 100f, CharacterStatModifierMode.PercentAdd, source);
                break;
            case StackStatType.Magic:
                runtimeStats.AddModifier(CharacterStatType.Magic, magicFlatIncrease, CharacterStatModifierMode.Flat, source);
                break;
            case StackStatType.MoveSpeed:
                runtimeStats.AddModifier(CharacterStatType.MoveSpeed, moveSpeedPercentIncrease / 100f, CharacterStatModifierMode.PercentAdd, source);
                break;
            case StackStatType.MaxHealth:
                runtimeStats.AddModifier(CharacterStatType.MaxHealth, maxHealthFlatIncrease, CharacterStatModifierMode.Flat, source);
                runtimeStats.RestoreHealth(maxHealthFlatIncrease);
                break;
        }
    }
    ...
    private static StatChoice MapToLegacyChoice(StackStatType statType)
    {
        return statType switch
        {
            StackStatType.AttackPower => StatChoice.AttackDefenseUp,
            StackStatType.AttackRange => StatChoice.AttackSpeedRangeUp,
            StackStatType.AttackSpeed => StatChoice.AttackSpeedRangeUp,
            StackStatType.Magic => StatChoice.MagicCooldownUp,
            StackStatType.MoveSpeed => StatChoice.SpeedUp,
            StackStatType.MaxHealth => StatChoice.HealthUp,
            _ => StatChoice.HealthUp
        };
    }
}
